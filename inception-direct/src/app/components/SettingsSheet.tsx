...
import { connect, deleteAllChats, setUi, store, updateSettings } from '../controller';
import { formatAgo } from '../format';
import { useStore } from '../store';
import type { ReadingFace, ThemeChoice, TransportChoice } from '../types';
...
const TRANSPORTS: { value: TransportChoice; label: string }[] = [
  { value: 'auto', label: 'Automatic' },
  { value: 'direct', label: 'Direct' },
  { value: 'bridge', label: 'Site tab' },
];
const STATUS_TEXT = {
  connecting: 'Connecting…',
  live: 'Live',
  challenge: 'Needs security check',
  verifying: 'Verifying…',
  offline: 'Offline',
  blocked: 'Blocked (not running as extension)',
  error: 'Error',
} as const;
      ...
      <aside className="sheet" data-open={open} aria-label="Settings" aria-hidden={!open} inert={!open}>
        ...
        <div className="sheet-body">
          ...
          <section>
              ...
              <div>
                ...
                <dd>
                  {connection.mode === 'direct'
                    ? `This browser → ${BASE_HOST}`
                    : connection.mode === 'bridge'
                      ? `This browser → ${BASE_HOST} tab → ${BASE_HOST}`
                      : 'Web page (blocked by CORS)'}
                </dd>
              </div>
              <div>
                <dt>Session</dt>
                <dd>
                  {connection.fetchedAt ? `created ${formatAgo(connection.fetchedAt)} · renews every 10 min` : 'none yet'}
                </dd>
              </div>
              <div>
                <dt>Tokens issued</dt>
            ...
            {runtime === 'extension' && (
              <div className="field">
                <span className="field-label">Route requests</span>
                <Segmented label="Route requests" value={settings.transport} options={TRANSPORTS} onChange={(transport) => updateSettings({ transport })} />
                <p className="field-help">
                  Automatic calls Inception directly and falls back to a {BASE_HOST} tab if direct calls are refused. Both run only in this browser.
                </p>
              </div>
            )}
