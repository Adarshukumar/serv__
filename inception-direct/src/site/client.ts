...
/**
 * Speaks the chat site's UI-message protocol using an injected fetch transport.
 * In the local companion, that transport is the dedicated same-origin Chrome page;
 * the browser's cookies and site token never leave the user's computer.
 */
...
export async function* readEventStream(
  ...
  /** A clean EOF without a finish/[DONE] is a truncated answer, not a success. */
  let ended = false;
  let finished = false;
  const onAbort = () => {
    reader.cancel().catch(() => {});
  ...
  try {
    for (;;) {
      ...
      for (const payload of sse.push(decoder.decode(chunk.value, { stream: true }))) {
        ...
        if (!event) continue;
        if (event.type === 'done') { finished = true; return; } // finally{} releases the connection
    ...
    if (!finished) throw new InceptionError('stream', 'The site stopped sending data before finishing the answer.');
  } finally {
