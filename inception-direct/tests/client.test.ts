...
import { InceptionClient } from '../src/site/client';
import { InceptionError } from '../src/site/errors';
import type { StreamEvent } from '../src/site/events';
import { SessionManager } from '../src/site/session';
...
describe('InceptionClient.chat', () => {
  ...
  it('flags a clean HTTP EOF without finish or [DONE] as a truncated answer', async () => {
    const { client } = setup(() => streamResponse([encoder.encode(sse({ type: 'text-delta', delta: 'Only the first half' }))]));
    const error = (await collect(client.chat({ chatId: 'c', turns: TURNS })).catch((e: unknown) => e)) as InceptionError;
    expect(error.kind).toBe('stream');
    expect(error.message).toContain('before finishing');
  });
